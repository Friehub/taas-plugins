...
exports.SportMonksDataSource = void 0;
 ...
 */
class SportMonksDataSource extends discovery_1.SovereignAdapter {
    constructor(config = {}) {
        super({
            ...config,
            id: 'sportmonks',
    ...
    async fetchData(params, signal) {
        ...
        const discoverFixtureId = async (signal) => {
            ...
            try {
                ...
                if (lp.data?.data?.[0]?.id)
                    return lp.data.data[0].id;
            }
            catch (e) { }
            ...
            try {
                ...
                if (sc.data?.data?.[0]?.id)
                    return sc.data.data[0].id;
            }
            catch (e) { }
        ...
        if (method === 'sports.football.score') {
            try {
                ...
                if (!f)
                    throw new Error(`[SportMonks] Fixture ${fixtureId} not found.`);
                return {
                    match_id: f.id,
                    home: f.name,
                    score: {
                        home: f.scores?.find((s) => s.description === 'CURRENT')?.score?.home || 0,
                        away: f.scores?.find((s) => s.description === 'CURRENT')?.score?.away || 0
                    },
...
exports.SportMonksDataSource = SportMonksDataSource;
