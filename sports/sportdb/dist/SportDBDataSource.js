...
exports.SportDBDataSource = exports.SportDBScoreSchema = void 0;
const discovery_1 = require("@taas/discovery");
const zod_1 = require("zod");
exports.SportDBScoreSchema = zod_1.z.object({
    idEvent: zod_1.z.string(),
    strEvent: zod_1.z.string(),
    intHomeScore: zod_1.z.union([zod_1.z.string(), zod_1.z.number()]).nullable(),
    intAwayScore: zod_1.z.union([zod_1.z.string(), zod_1.z.number()]).nullable(),
    strStatus: zod_1.z.string(),
    timestamp: zod_1.z.number().optional()
});
class SportDBDataSource extends discovery_1.SovereignAdapter {
    getMockData(params) {
        return {
    ...
    constructor(config = {}) {
        super({
            name: 'SportDB',
            category: discovery_1.DataCategory.SPORTS,
    ...
    async resolveMatchId(params, signal) {
        const adapter = this;
        ...
        if (params.homeTeam && params.awayTeam) {
            ...
            try {
                ...
                const event = response.data?.event?.find((e) => e.strHomeTeam.toLowerCase().includes(params.homeTeam.toLowerCase()) &&
        ...
        if (params.teamName || params.homeTeam) {
            ...
            try {
                ...
                if (teamId) {
                    ...
                    if (nextRes.data?.events?.[0])
                        return nextRes.data.events[0].idEvent;
                    ...
                    if (lastRes.data?.results?.[0])
                        return lastRes.data.results[0].idEvent;
                }
            }
            catch (e) { }
        }
    ...
    async fetchData(params, signal) {
        ...
        if (method === 'livescore' || method === 'sports.football.list') {
            const response = await adapter.client.get(`https://www.thesportsdb.com/api/v1/json/${apiKey}/latestsoccer.php`, { signal });
            if (!response.data || !response.data.teams) {
                return { status: "Empty", matches: [], timestamp: Date.now() };
            }
            if (method === 'sports.football.list') {
                return {
                    matches: (response.data.teams.Match || []).map((m) => ({
                        id: m.Id,
                        home: m.HomeTeam,
                        away: m.AwayTeam,
                        league: m.League,
        ...
        if (method === 'sports.league.list') {
            ...
            if (params.sportName) {
                leagues = leagues.filter((l) => l.strSport.toLowerCase() === params.sportName?.toLowerCase());
            }
            return { leagues: leagues.map((l) => ({ id: l.idLeague, name: l.strLeague, sport: l.strSport, alternate: l.strLeagueAlternate })), timestamp: Date.now() };
        }
        ...
        if (method === 'sports.league.detail') {
            ...
            if (!l)
                return { league: null, timestamp: Date.now() };
            return {
                league: {
                    id: l.idLeague,
                    name: l.strLeague,
        ...
        if (method === 'sports.team.list') {
            ...
            return { teams: (response.data?.teams || []).map((t) => ({ id: t.idTeam, name: t.strTeam, badge: t.strTeamBadge, stadium: t.strStadium, venue: t.strVenue })), timestamp: Date.now() };
        }
        ...
        if (method === 'sports.player.list') {
            ...
            return { players: (response.data?.player || []).map((p) => ({ id: p.idPlayer, name: p.strPlayer, nationality: p.strNationality, position: p.strPosition, thumb: p.strThumb })), timestamp: Date.now() };
        }
        ...
        if (method === 'sports.league.upcoming') {
            ...
            return { events: (response.data?.events || []).map((ev) => ({ id: ev.idEvent, match: ev.strEvent, date: ev.dateEvent, time: ev.strTime, home: ev.strHomeTeam, away: ev.strAwayTeam })), timestamp: Date.now() };
        }
        ...
        if (method === 'sports.team.results') {
            ...
            return {
                results: (response.data?.results || []).map((ev) => ({
                    id: ev.idEvent,
                    match: ev.strEvent,
                    date: ev.dateEvent,
                    home: ev.strHomeTeam,
        ...
        if (method === 'sports.player.detail') {
            ...
            if (!playerId)
                throw new Error("Missing playerId for sports.player.detail");
            ...
            if (!p)
                return { player: null, timestamp: Date.now() };
            return {
                player: {
                    id: p.idPlayer,
                    name: p.strPlayer,
        ...
        if (method === 'sports.player.honors') {
            const playerId = params.playerId;
            if (!playerId)
                throw new Error("Missing playerId for sports.player.honors");
        ...
        let liveMatch = (liveResponse.data?.teams?.Match || []).find((m) => String(m.Id) === String(id));
        let e;
        if (liveMatch) {
            e = liveMatch;
        }
        ...
        if (needsDetails) {
            ...
            if (isFootball) {
                ...
                const parseEvents = (str, side, type = 'Goal') => {
                ...
                const parseLineup = (val) => (typeof val === 'string' ? val : '').split(';').map(s => s.trim()).filter(Boolean);
                ...
                result.details.stats = {
                    ...
                    yellow_cards: result.details.yellow_cards.filter((c) => c.side === 'home').length,
                    red_cards: result.details.red_cards.filter((c) => c.side === 'home').length
                };
...
exports.SportDBDataSource = SportDBDataSource;
